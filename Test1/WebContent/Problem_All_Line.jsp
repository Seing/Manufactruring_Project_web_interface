...
function go_pop(Name){
	//alert(Name);
	window.open("predict.jsp?text_L="+a+"&text_P="+b+"&name="+Name,"new","width=1200, height=800, resizable=yes, scrollvars=yes, status=no, location=no, directories=no;");
}
    ...
    $('#container').highcharts({
        ...
        series: [{
            ...
            data: [
				   ...
				   String strSQL1 = "SELECT DISTINCT Problem FROM List_All";   //고장 원인 코드를 중복값 제거하여 가져오는 쿼리문
				   ...
				   while(rs.next()){                 //고장 원인 코드를 가져와 ArrayList에 저장
					   	...
					   	System.out.println("code ="+Problem_code.size());
				   ...
				   for(int i=0; i<Problem_code.size(); i++){
					   String strSQL2 = "SELECT COUNT(Problem) FROM List_All WHERE Problem = \"" + Problem_code.get(i)+ "\"";   //각 고장 원인의 빈도수를 가져오는 쿼리문
